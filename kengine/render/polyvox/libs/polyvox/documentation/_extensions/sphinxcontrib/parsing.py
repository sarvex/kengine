...
input_type = Combine(
	Word(f'{alphanums}:_')
	+ Optional(angle_bracket_pair + Optional(Word(f'{alphanums}:_')))
)
...
arglist = (
	LPAR
	+ delimitedList(argument)('arg_list')
	+ Optional(f'{COMMA}...')('var_args')
	+ RPAR
)
...
def normalise(symbol):
	...
	try:
		...
		print(f'Could not find closing bracket in {arglist_input_string}')
		raise
	...
	try:
		...
		for arg in result.arg_list:
			...
			const_pointer_ref_list = [arg.pointer_or_reference1]
...
def normalise_list(list_of_symbols):
	return itertools.imap(normalise, list_of_symbols)
